...
import { PAGE_ACTIVITY_APPLICATION } from '@constants/page'
import { PAYLOAD_ACTIVITY_CREATE } from '@constants/api'
...
class activityApply extends BaseComponent {
  config = {
  ...
  state = {
    ...
    inputQQ: '',
    inputTel: '',
    payload: PAYLOAD_ACTIVITY_CREATE,
  }
  ...
  async componentWillMount() {
    ...
    this.setState({
      ...
      payload: { ...payload, activity_id: id,apply_id:user.Member.id,apply_name:'user.Member.name' }
    })
  }
  //输入姓名
  ...
  onAppilcation() {
    const { payload } = this.state
    this.onCheck() && this.props.dispatchActivityCreate(payload)
    this.onCheck() && Taro.showToast({
      title: '报名成功',
      icon: 'none',
      duration: 2000,
    })
  ...
  render() {
    ...
    return (<View >
      ...
      <View className='at-row at-row__justify--center mt-4'>
        ...
        <AtButton type='primary' circle style='width:100vw' className='m-2' onClick={this.onAppilcation}> 提交报名   </AtButton>
