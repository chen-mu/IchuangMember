...
import { PAYLOAD_MATCH_CREATE } from '@constants/api'
...
class activityApply extends BaseComponent {
  ...
  async componentWillMount() {
    ...
    this.setState({
      id,
      name,
      payload: { ...payload, competition_id: id }
    })
  }
  //输入姓名
  ...
  onAppilcation() {
    const { payload } = this.state
    this.onCheck() && this.props.dispatchMatchCreate(payload).then((res)=>{
      if(res.data.total === 0){
        Taro.showToast({
          title: '报名成功',
          icon: 'none',
