...
import { PAGE_MATCH_APPLICATION } from '@constants/page'
...
class matchDetail extends BaseComponent {
  config = {
    navigationBarTitleText: '大赛',
  ...
  async componentWillMount() {
    ...
    const { data } = await this.props.dispatchMatchDetail({ id })
  ...
  onAppilcation(){
    const { id,name } = this.state
      Taro.navigateTo({
        url:`${PAGE_MATCH_APPLICATION}?id=${id}&name=${name}`
      })
  }
  render() {
